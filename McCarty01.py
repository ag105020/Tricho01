...
def McCarty01(ep):
    ...
    #    dgpc=18.8       #dg for the production of BB (bacterial biomass) from pyruvate)
    #    dgpc=3.33*1/20*(12*5+1*7+16*2+14*1)
    #    print(dgpc)
    ...
    NtoC=1*C/(-S4[2,5])*S4[0,0]/(fn00*dgn+fs00*(dgp/ep1+dgpc/ep))*(fn00*dgn)            #(molC/molN) C to N conversion
    RestoBio = 1/(-S4[1,8]*n) * (C*S4[0,0]/(fn00*dgn+fs00*(dgp/ep1+dgpc/ep))*fs00*(dgp/ep1+dgpc/ep) - S4[1,2] - S4[1,4])
    
#     print(1/(fn00*dgn+fs00*(dgp/ep1+dgpc/ep))*fs00*(dgp/ep1+dgpc/ep))
#     print(fn00*dgn)
#     print(fs00*(dgp/ep1+dgpc/ep))
